...
if ($Latest_Version -eq $Current_Version) {
    ...
    $Latest_Linux64_Hash = curl -Ls "https://github.com/v2fly/v2ray-core/releases/download/v$Latest_Version/v2ray-linux-64.zip.dgst" | grep '256= ' | awk -F ' ' '{print $2}'
    $Latest_macOS_x64Hash = curl -Ls "https://github.com/v2fly/v2ray-core/releases/download/v$Latest_Version/v2ray-macos-64.zip.dgst" | grep '256= ' | awk -F ' ' '{print $2}'
    $Latest_macOS_arm64Hash = curl -Ls "https://github.com/v2fly/v2ray-core/releases/download/v$Latest_Version/v2ray-macos-arm64-v8a.zip.dgst" | grep S'256= ' | awk -F ' ' '{print $2}'
