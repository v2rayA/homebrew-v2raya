...
if [ $Latest_Version == $Current_Version ]; then
    ...
    Latest_Linux64_Hash=$(curl -Ls https://github.com/v2fly/v2ray-core/releases/download/v$LatestVersion/v2ray-linux-64.zip.dgst | grep SHA256 |awk -F ' ' '{print $2}')
    Latest_macOS_x64Hash=$(curl -Ls https://github.com/v2fly/v2ray-core/releases/download/v$LatestVersion/v2ray-macos-64.zip.dgst | grep SHA256 |awk -F ' ' '{print $2}')
    Latest_macOS_arm64Hash=$(curl -Ls https://github.com/v2fly/v2ray-core/releases/download/v$LatestVersion/v2ray-macos-arm64-v8a.zip.dgst | grep SHA256 |awk -F ' ' '{print $2}')
